...
Object remakeLineP2(Object smfis, Object scv, Object rem, int id, double ang, int refpoint)
{
	...
	Object result(x1,y1,x2,y2, id, 0, rem.getP1OS(), rem.getP2OS(), 1);
        ...
        //modified by lyn
        result.setKP(1);
        result.setASRNo(rem.getASRNo());
        result.setChunkFLag(rem.getChunkFlag());
        result.setColorID(rem.getColorID());
        //s.setGID(addView[i].getGID());
        //s.setID(addView[i].getID());
        result.setKP(rem.getKP());
        result.setLimitingPoint(rem.getLimitingPoint());
        result.setLocalEnvID(rem.getLocalEnvID());
        //s.setLocalEnvID();
        //s.setNS();
        result.setOoPV(rem.getOoPV());
        result.setOrt(rem.getOrt());
        result.setP1OS(rem.getP1OS());
        result.setP2OS(rem.getP2OS());
        //result.setPEP1(addView[i].getPEP1());
        //result.setPEP2(addView[i].getPEP2());
        result.setPO(rem.getPO());
        result.setPos(rem.getPos());
        result.setVN(rem.getVN());
...
vector<Object> projectingTheView_ondiff_point(vector<Object> currentView, Object refObjectOld, Object refObjectNew, int refPoint) 
{
    ...
    for (unsigned int i = 0; i < currentView.size(); i++) 
    {
        temp = remakeLine_ondiff_point(refObjectOld, refObjectNew, currentView[i], currentView[i].getID(), 0,refPoint);
        result.push_back(temp);
    }
...
vector<Object> projectingTheView_with_correctDirection(vector<Object> currentView, Object refObjectOld, Object refObjectNew, int refPoint) 
{
    ...
    if(abs(refObjectOld.getAngleWithLine(refObjectNew) > 170.0) 
            && abs(refObjectOld.getAngleWithLine(refObjectNew) < 189.0))
    {
        if((distanceOftwoP(refObjectOld.getP1(), refObjectNew.getP1()) + distanceOftwoP(refObjectOld.getP2(), refObjectNew.getP2())) > (refObjectOld.length()*1.5))
            refObjectNew.set(refObjectNew.X2(), refObjectNew.Y2(), refObjectNew.X1(), refObjectNew.Y1(), refObjectNew.getID());
    }
    ...
    {
        result[i].set_match_label(currentView[i].get_match_label());
    }
...
vector<Exit> projectingTheExit(vector<Exit> exits, Exit ref_oldExit, Exit ref_newExit, int refPoint) 
{
    ...
    Object temp, temp_obj;
    Object refObjectOld, refObjectNew;
    vector<Object> currentView;
    vector<Object> intermediate; 
    
    //convert to object class
    for(int i = 0; i < exits.size(); i++)
    {
        temp_obj.set(exits[i].X1(), exits[i].Y1(), exits[i].X2(), exits[i].Y2(), i);
        currentView.push_back(temp_obj);
    }
    
    refObjectOld.set(ref_oldExit.X1(), ref_oldExit.Y1(), ref_oldExit.X2(), ref_oldExit.Y2(), 0);
    refObjectNew.set(ref_newExit.X1(), ref_newExit.Y1(), ref_newExit.X2(), ref_newExit.Y2(), 0);
    ...
    //convert those back to exit class
    for(int i = 0; i < intermediate.size(); i++)
    {
        temp_exit.set(intermediate[i].X1(), intermediate[i].Y1(), intermediate[i].X2(), intermediate[i].Y2());
        result.push_back(temp_exit);
    }
