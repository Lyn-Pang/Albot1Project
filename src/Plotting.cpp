...
const unsigned int PLOT_RESOLUTION_X = 3000;//2400;
const unsigned int PLOT_RESOLUTION_Y = 3000;//2400;
...
void plotObjectsOf3Kinds(const char * filename, vector<Object> pose, vector<Object> Objects1, vector<Object> Objects2) {
    ...
    fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth %d\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y,11);
    fprintf(fgnup, "set output \"%s\"\n", filename);
    fprintf(fgnup, "set yrange[%g:%g]\n", minY, maxY);
    fprintf(fgnup, "set xrange[%g:%g]\n", minX, maxX);
    //fprintf(fgnup, "set yrange[%d:%d]\n", 10000, 20000);//for thesis first view
    //fprintf(fgnup, "set xrange[%d:%d]\n", -7000, 3000);
...
void plotObjectsOf4Kinds(const char * filename, vector<Object> pose,
    ...
    for (int i = 0; i<int(nodes.size()); i++) {
        minX = min(minX, nodes[i].X1());
        minX = min(minX, nodes[i].X2());
        maxX = max(maxX, nodes[i].X1());
        maxX = max(maxX, nodes[i].X2());
        minY = min(minY, nodes[i].Y1());
        minY = min(minY, nodes[i].Y2());
        maxY = max(maxY, nodes[i].Y1());
        maxY = max(maxY, nodes[i].Y2());
    }
    ...
    fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth 11\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y);
    //fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth 10\n", 3000, 3000);
    fprintf(fgnup, "set output \"%s\"\n", filename);
    fprintf(fgnup, "set yrange[%g:%g]\n", minY, maxY);
    fprintf(fgnup, "set xrange[%g:%g]\n", minX, maxX);
...
void plotObjectsOf5Kinds(const char * filename, vector<Object> pose,
        vector<Object> nodes,
        vector<Object> Objects1,
        vector<Object> Objects2,
        vector<Object> Objects3) 
{
    FILE * fgnup = popen(GNUPLOT_PATH, "w");
    if (!fgnup) {
        cerr << "ERROR: " << GNUPLOT_PATH << " not found" << endl;
    ...
    for (int i = 0; i<int(Objects3.size()); i++) {
        minX = min(minX, Objects3[i].X1());
        minX = min(minX, Objects3[i].X2());
        maxX = max(maxX, Objects3[i].X1());
        maxX = max(maxX, Objects3[i].X2());
        minY = min(minY, Objects3[i].Y1());
        minY = min(minY, Objects3[i].Y2());
        maxY = max(maxY, Objects3[i].Y1());
        maxY = max(maxY, Objects3[i].Y2());
    }
...
//    fprintf(fgnup, "set xrange[%d:%d]\n", -10000, 4000);
 ...
 //               fprintf(fgnup, "set xrange[%d:%d]\n", -7500, 1500);
    ...
    for (int i = 0; i<int(Objects3.size()); i++) {
        fprintf(fgnup, "%g ", Objects3[i].X1());
        fprintf(fgnup, "%g\n", Objects3[i].Y1());
        fprintf(fgnup, "%g ", Objects3[i].X2());
        fprintf(fgnup, "%g\n\n", Objects3[i].Y2());
    }
    fprintf(fgnup, "e\n");
...
void plotObjectsOf3KindswithExits(const char * filename, vector<Object> pose, vector<Object> Objects1, vector<Exit> Objects2) 
{
    ...
    fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth %d\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y,8);
    fprintf(fgnup, "set output \"%s\"\n", filename);
    fprintf(fgnup, "set yrange[%g:%g]\n", minY, maxY);
    fprintf(fgnup, "set xrange[%g:%g]\n", minX, maxX);
//    fprintf(fgnup, "set yrange[%d:%d]\n", -1500, 7000);//for thesis first view
...
//    }
    ...
    fprintf(fgnup, "set object 1 circle at 0,0 size %g\\\n ", rad+500);
    fprintf(fgnup, "arc [30:80]\n");

    fprintf(fgnup, "set object 2 circle at 0,0 size %g\\\n ", rad+1500);
    fprintf(fgnup, "arc [110:180]\n");
    
    fprintf(fgnup, "plot \"-\" ti \"Robot Positions\" with linespoint 1, \\\n"); //for first argument
    fprintf(fgnup, "\"-\" ti \"Target Objects\" with linespoint 1, \n"); //for second argument
    //fprintf(fgnup, "\"-\" ti \"Objects\" with lines 3\n"); //for third argument
...
//    }
    ...
    for (int i = 0; i<int(Objects2.size()); i++) 
    {
        fprintf(fgnup, "%g ", origin.X());
        fprintf(fgnup, "%g\n", origin.Y());
        fprintf(fgnup, "%g ", Objects2[i].X());
        fprintf(fgnup, "%g\n\n", Objects2[i].Y());
    }
    fprintf(fgnup, "e\n");
...
//    }
    ...
    // Plot Objects - 3rd argument
    for (int i = 0; i<int(Objects2.size() - 1); i++) 
    {
        fprintf(fgnup, "%g ", Objects2[i].X());
        fprintf(fgnup, "%g\n", Objects2[i].Y());
        fprintf(fgnup, "%g ", Objects2[i+1].X());
        fprintf(fgnup, "%g\n\n", Objects2[i+1].Y());
    }
    fprintf(fgnup, "e\n");
...
void plotObjectsOf3KindsDifColor(const char * filename, vector<Object> pose, vector<Object> Objects1, vector<Object> Objects2)
{
    ...
    fprintf(fgnup, "plot \"-\" ti \"Robot Positions\" with linespoint 1, \\\n"); //for first argument
    fprintf(fgnup, "\"-\" ti \"Target Objects\" with linespoint %d, \\\n", NUMB++); //for second argument
    fprintf(fgnup, "\"-\" ti \"Objects\" with lines 3\n"); //for third argument
...
/*Plotting all views on one coordination 
 *with different colours for distinguish
 * the input variable is vector< vector<Object> > type
 */
void plotObjectsColours(const char * filename, vector<vector<Object> > pose) 
{
        ...
        fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth 9\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y);
        fprintf(fgnup, "set output \"%s\"\n", filename);
        fprintf(fgnup, "set yrange[%g:%g]\n", minY, maxY);
        fprintf(fgnup, "set xrange[%g:%g]\n", minX, maxX);
        //fprintf(fgnup, "set yrange[%g:%g]\n", -25000.0, 11000.0);
        ...
        //cout<<"y range minY: "<<minY<<" ; maxY: "<<maxY<<endl;
        //cout<<"y range minX: "<<minX<<" ; maxX: "<<maxX<<endl;
        //waitHere();
        for (unsigned int i = 0; i < pose.size(); i++) 
        {
                //fprintf(fgnup, "\"-\" ti \"Robot Positions\" with lines %d", i + 2); //for second argument
                if(i >= 24 && i<= 31)
                      i += 10;
                fprintf(fgnup, "\"-\" with lines %d", i + 1); //for second argument
                if(i < pose.size() - 1)
                        fprintf(fgnup, ", \\\n");
                else
                        fprintf(fgnup, ", \n");
...
//another version with robot positon 
void plotObjectsColours(const char * filename, vector<vector<Object> > pose, vector<Object> rbp) 
{
        ...
        fprintf(fgnup, "\"-\" ti \"Robot Positions\" with lines %d", 1); //for second argument
        fprintf(fgnup, ", \\\n");
        ...
        for (unsigned int j = 0; j < rbp.size(); j++) 
        {
                    fprintf(fgnup, "%g ", rbp[j].X1());
                    fprintf(fgnup, "%g\n", rbp[j].Y1());
                    fprintf(fgnup, "%g ", rbp[j].X2());
                    fprintf(fgnup, "%g\n\n", rbp[j].Y2());
        }
        fprintf(fgnup, "e\n");
...
void plotObjectsOf4KindsDottedLandMarks(const char * filename, vector<Object> pose,
        ...
        vector<Object> Objects2) 
{
    //process landmark surfaces, and delete duplicated surfaces
    for( int i = 0; i < Objects2.size(); i++)
    {
        for(int j = 0; j < pose.size(); j++)
        {
            if(pose[j].X1() == Objects2[i].X1() && pose[j].X2() == Objects2[i].X2())
            {
                pose.erase(pose.begin()+j);
                j = 0;
            }
        }
    }
    ...
    for( int i = 0; i < Objects2.size(); i++)
    {
        ...
        {
            if(Objects1[j].X1() == Objects2[i].X1() && Objects1[j].X2() == Objects2[i].X2())
            {
                Objects1.erase(Objects1.begin()+j);
                j = 0;
            }
        }
    }
    ...
    //fprintf(fgnup, "set terminal pngcairo dashed size%d,%d\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y);
    //fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth 11\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y);
    fprintf(fgnup, "set terminal png size %d,%d nocrop linewidth 11\n", PLOT_RESOLUTION_X, PLOT_RESOLUTION_Y);
    ...
    //fprintf(fgnup, "set style line 4 linetype 3\n");
    //fprintf(fgnup, "set style line 3 lt 1 lw 11\n");
    //fprintf(fgnup, "set style line 2 lt 1 lw 11\n");
    //fprintf(fgnup, "set style line 1 lt 1 lw 11\n");
    

    fprintf(fgnup, "plot \"-\" ti \"Objects\" with linespoint 1, \\\n"); //for first argument
    fprintf(fgnup, "\"-\" ti \"Target Objects\" with lines 1, \\\n"); //for second argument
    fprintf(fgnup, "\"-\" ti \"Target Objects\" with lines 1, \\\n"); //for third argument
    fprintf(fgnup, "\"-\" ti \"Robot Positions\" with linespoint 2 linetype 3\n"); //for forth argument
    
    //fprintf(fgnup, "set style line 4 linetype 2\n");
...
*/
    ...
    fprintf(fgnup, "set object 1 ellipse at %g,%g size %g, %g angle %g\n ", centre.X(), centre.Y(), w, h, orientation);
                                            ...
                                            vector<Object> rbp, vector<Object> Objects2) 
{
        ...
        for (int i = 0; i<int(rbp.size()); i++)
        {
            minX = min(minX, rbp[i].X1());
            minX = min(minX, rbp[i].X2());
            maxX = max(maxX, rbp[i].X1());
            maxX = max(maxX, rbp[i].X2());
            minY = min(minY, rbp[i].Y1());
            minY = min(minY, rbp[i].Y2());
            maxY = max(maxY, rbp[i].Y1());
            maxY = max(maxY, rbp[i].Y2());
        }
        ...
        fprintf(fgnup, "\"-\" ti \"Object Positions\" with lines %d", 115); //for second argument
        fprintf(fgnup, ", \n");
                                                       ...
                                                       vector<Exit> exits) 
{
        ...
        fprintf(fgnup, "\"-\" ti \"Exit Positions\" with lines %d", 28); //for exit argument
        fprintf(fgnup, ", \\\n");
        ...
        for (int i = 0; i<int(exits.size()); i++)
        {
            fprintf(fgnup, "%g ", exits[i].X1());
            fprintf(fgnup, "%g\n", exits[i].Y1());
            fprintf(fgnup, "%g ", exits[i].X2());
            fprintf(fgnup, "%g\n\n", exits[i].Y2());
        }
        fprintf(fgnup, "e\n");
...
*/
    ...
    {
        fprintf(fgnup, "set object %d ellipse at %g,%g size %g, %g angle %g\n ", i,
               Ellipses[i].getCentre().X(), Ellipses[i].getCentre().Y(), 
                Ellipses[i].getwidth()/2, Ellipses[i].getHeigth(), Ellipses[i].getOrientation());
    }
    fprintf(fgnup, "plot \"-\" ti \"Robot Positions\" with linespoint 1, \\\n"); //for first argument
    fprintf(fgnup, "\"-\" ti \"Target Objects\" with linespoint 2, \n"); //for second argument
    //fprintf(fgnup, "\"-\" ti \"Objects\" with lines 3\n"); //for third argument
