...
//============================================================================
// Name        : Path Simplification and Expectation 
// Author      : Wenwang (Lyn)
// Version     :
...
vector<Object> rest_of_info; //except expected informaiton
...
lb_pi:                          Object aLine = makeLineAtTwoPointsWithObject(coordTransInfo[1], coordTransInfo[0], coordTransInfo[1], coordTransInfo[0] + 500, currentRobotPositionInMFIS[6], 1);
                                ...
                                //modified part
                                //match with MFIS
                                vector<Object> transformed_view = TransformforToGlobalCoordinate(currentView, odometricCRPositionInMFIS[6].getP1(), odometricCRPositionInMFIS, odometricCRPositionInMFIS[7].getAngleWithXaxis());
                                currentRobotPositionInMFIS = myrobot.inMFIS(referenceObjects[0], referenceObjects[1], referenceObjects[0].getKP());
                                //int match_num = match_two_views(MFIS, transformed_view);
                                //double percent = double (match_num) / double(currentView.size());
                                
                                if(expect_process_flag == 0)
                                {
                                    
                                    
                                        adjust_info.first = MFIS;
                                        adjust_info.second = currentRobotPositionInMFIS;
                                
                                    if(abs(robotTurning) > 50)
                                    {
                                        fixed_mfis = MFIS;
                                        adjust_info = info_adjustment(transformed_view, odometricCRPositionInMFIS, MFIS, adjust_info.first, v);
                                        sprintf(viewFileName, "%s%d%s", "Maps/Offline/Transed_and_MFIS", v, ".png");
                                        plotObjectsOf5Kinds(viewFileName, transformed_view, odometricCRPositionInMFIS, adjust_info.first, adjust_info.second, MFIS);
                                        
                                        currentRobotPositionInMFIS = adjust_info.second;
                                        MFIS = adjust_info.first;
                                        rest_of_info = remaining;
                                        /*//vector< vector<Object> > corrected_info = Current_and_expectation(transformed_view, currentRobotPositionInMFIS, MFIS, v);
                                        
                                        vector< vector<Object> > corrected_info = structure_align(transformed_view, currentRobotPositionInMFIS, MFIS, v, info_part_flag);
                                        MFIS = corrected_info[0];
                                        transformed_view = corrected_info[2];
                                        currentRobotPositionInMFIS = corrected_info[1];
                                        sprintf(viewFileName, "%s%d%s", "Maps/Offline/Transed_and_MFIS", v, ".png");
                                        plotObjectsOf4Kinds(viewFileName, corrected_info[2], corrected_info[1], corrected_info[0], MFIS);*/
                                        PI_switch_Flag = 1;
                                        
                                        if(abs(robotTurning) >= 180)
                                            robotTurning = 0;
                                    }
                                ...
                                {
                                    //adjust_info = info_adjustment_next(transformed_view, currentRobotPositionInMFIS, MFIS, rest_of_info, v);
                                    vector< vector<Object> > corrected_info;
                                    //corrected_info = info_adjustment_next(transformed_view, currentRobotPositionInMFIS, MFIS, rest_of_info, v);
                                    corrected_info = info_adjustment_next_version(transformed_view, currentRobotPositionInMFIS, MFIS, rest_of_info, v, info_part_flag);
                                    sprintf(viewFileName, "%s%d%s", "Maps/Offline/Transed_and_MFIS", v, ".png");
                                    plotObjectsOf4Kinds(viewFileName, corrected_info[2], corrected_info[1], corrected_info[0], rest_of_info);
                                    currentRobotPositionInMFIS = corrected_info[1];
                                    MFIS = corrected_info[0];
                                    ...
                                    plotObjectsOf3Kinds(mfisFileName, allRobotPositions, rest_of_info, MFIS);
                                    goto lb_env;
                                }
        ...
        {
lb_env:     targetObjectsInPV = recognizedTargetObjects.getTargetObjects();
