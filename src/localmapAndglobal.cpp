...
int main()
{
    ...
    do
    {
        ...
        if ((recognizedTargetObjects.getTargetObjects().size() < 4))
        {
            ...
            plotObjectsOf3Kinds(mfisFileName, MFIS, myrobot.getRobot(), currentRobotPositionInMFIS);
    ...
    //for returning home, write date to files
    ...
    char LocalRobotPsFile[80];
    char TurningRobotPsFile[80];
    char UpdatingNumberFile[80];
    ...
    sprintf(LocalRobotPsFile, "%s%s", memFilePath, "LocalRobotPs.txt");
    sprintf(TurningRobotPsFile, "%s%s", memFilePath, "TurningRobotPs.txt");
    sprintf(UpdatingNumberFile, "%s%s", memFilePath, "UpdatingNumber.txt");
    sprintf(ReturnLocalPosition, "%s%s", memFilePath, "ReturnPositions.txt");
    
    vector<Point> AllGlobalRobotPoints;
    vector<double> AllGlobalRobotXAngles;


    for (int i = 0; i < AllGlobalRobotPs.size(); i++) // the ID of limiting point ---------vector<int> updatingNumber;
    {
        AllGlobalRobotXAngles.push_back(AllGlobalRobotPs[i][7].getAngleWithXaxis());
        AllGlobalRobotPoints.push_back(Point(AllGlobalRobotPs[i][6].X1(), AllGlobalRobotPs[i][6].Y1()));
    }
    writePositions(AllGlobalRobotPoints, AllGlobalRobotPsFile);
    writeNumbers(AllGlobalRobotXAngles, AllGlobalRobotXAngleFile);


    
    // For printing coordinate positions and limiting point //
    for (int i = 0; i < RobotPs.size(); i++) // all global positions ----------------------vector<Point> RobotPs;
    {
        cout << "positons x:  " << RobotPs[i].X() << "; y: " << RobotPs[i].Y() << endl;
    }
    writePositions(RobotPs, PathPointGlobalRobotPsFile);
    //write the global robot positions into a file named "GlobalRobotPs.txt".

    for (int i = 0; i < rbs.size(); i++) // all relative local positions   --------------------vector<Point> rbs;
    {
        cout << "local coordinate rob positions x:  " << rbs[i].X() << "; y: " << rbs[i].Y() << endl;
    }
    writePositions(rbs, LocalRobotPsFile); //write the local robot positions into a file named "localRobotPs.txt".

    for (int i = 0; i < turningPoint.size(); i++) // all relative turning positions -------vector<Point> turningPoint; 
    {
        cout << "turning point rob positions x:  " << turningPoint[i].X() << "; y: " << turningPoint[i].Y() << endl;
    }
    writePositions(turningPoint, TurningRobotPsFile); //write the local robot positions into a file named "localRobotPs.txt".

    for (int i = 0; i < updatingNumber.size(); i++) // the ID of limiting point ---------vector<int> updatingNumber;
    {
        cout << "updatingNumber is: " << updatingNumber[i] << endl;
    }
    writeNumbers(updatingNumber, UpdatingNumberFile);

    vector<double> PathPointRobotXAngles;
    for (int i = 0; i < ListOfPositions.size(); i++) // the ID of limiting point ---------vector<int> updatingNumber;
    {
        PathPointRobotXAngles.push_back(ListOfPositions[i][7].getAngleWithXaxis());
    }
    writeNumbers(PathPointRobotXAngles, PathPointGlobalRobotXAngleFile);

    ReturningRobotPs = returningPositions(RobotPs, PathPointRobotXAngles); // calculated returning relative positions
    for (int i = 0; i < ReturningRobotPs.size(); i++) // the ID of limiting point
    {
        cout << "test x is: " << ReturningRobotPs[i].X() << "; y: " << ReturningRobotPs[i].Y() << endl;
    }
    writePositions(ReturningRobotPs, ReturnLocalPosition);
    
   
    //vector<Object> testStructure;
    //testStructure = StructureOfChunk(allChunks[1].getChunkView());
