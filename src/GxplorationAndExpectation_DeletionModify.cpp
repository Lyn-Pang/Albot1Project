...
lb_pi:                          Object aLine = makeLineAtTwoPointsWithObject(coordTransInfo[1], coordTransInfo[0], coordTransInfo[1], coordTransInfo[0] + 500, currentRobotPositionInMFIS[6], 1);
            ...
            vector<Object> test_rob;
            test_rob = currentRobotPositionInMFIS;
            ...
            if(v == 142)
            {
                currentRobotPositionInMFIS = TransformforToGlobalCoordinate(currentRobotPositionInMFIS, Point (-700,300), currentRobotPositionInMFIS, 8);  
            }
            if(v == 204)
            {
                currentRobotPositionInMFIS = TransformforToGlobalCoordinate(currentRobotPositionInMFIS, Point (700,-200), currentRobotPositionInMFIS, 5);  
            }
...
//                        sprintf(mfisFileName, "%s%d%s", "Maps/Offline/MFIS_to_critical_point-", v, ".png");                   
//                        plotObjectsOf3Kinds(mfisFileName, myrobot.getRobot(), currentRobotPositionInMFIS, environment_map);
...
re_adjust:              if(fist_step_m2 == 0)
                ...
                if(expectation_flag == 1)
                {
                    ...
                    polygon_area = convertObjectToSurface(area_of_new_view);
                    for(int m = 0; m < environment_map.size(); m++)
                    {
                        if((interSectWithLine(area_of_new_view, environment_map[m].getP1(), environment_map[m].getP2()) == true)
                            || ((pointInPolygon(PointXY(environment_map[m].X1(), environment_map[m].Y1()), polygon_area) == true) && (pointInPolygon(PointXY(environment_map[m].X2(), environment_map[m].Y2()), polygon_area) == true)))
                            threshold_cnt++;
                    ...
                    {
                        if(Intersect_number(environment_map, transformed_view[i]) > 0)
                            intersection_cnt++;
                    }

                    
                    if(((threshold_cnt <= 3) && (currentView.size() > 4)) || (intersection_cnt == 0))
                    {
                        // switch back to mathod 1 //
                        cout << " ----- Switch back to method 1 ----- " << endl;
                        //waitHere();
